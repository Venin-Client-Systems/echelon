...
export function formatEventForTelegram(event: EchelonEvent): string | null {
  switch (event.type) {
    ...
    case 'message': {
      ...
      const narrative = event.message.content
        .replace(/```json[\s\S]*?```/g, '')
        .replace(/\n{3,}/g, '\n\n')
        .trim()
        .slice(0, 300);
      return `\u{1F4E8} <b>${from} \u{2192} ${to}</b> (${cost}, ${duration})\n\n${escapeHtml(narrative)}${narrative.length >= 300 ? '...' : ''}`;
    }
    case 'action_pending':
    ...
    case 'cheenoski_slot_fill':
      return `\u{1F527} Engineer slot filled: #${event.slot.issueNumber} ${escapeHtml(event.slot.issueTitle)} [${event.slot.domain}]`;
    case 'cheenoski_slot_done': {
      const status = event.slot.status === 'done' ? '\u{2705}' : '\u{274C}';
      return `${status} Engineer #${event.slot.issueNumber}: ${event.slot.status}${event.slot.error ? ` \u{2014} ${escapeHtml(event.slot.error.slice(0, 100))}` : ''}`;
    }
